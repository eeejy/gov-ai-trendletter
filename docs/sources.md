...
공식 API 를 쓰지 않고 검색 결과 페이지의 `ytInitialData` 를 읽는다. API 키가 필요 없다.

```
https://www.youtube.com/results?search_query={제목}
```

- **기간 필터(`sp=CAISBAgCEAE%3D`)를 붙이면 결과가 0건**이 된다(2026-08-30 확인).
  기본 관련도 정렬을 쓰고 오래된 영상은 아래 조건으로 거른다
- 항목 제목에서 대괄호·괄호 안 부연을 떼어 검색어로 쓴다
- **제목 낱말이 2개 이상 겹칠 때만** 채택한다. 검색 상위라도 주제가 다른 영상이 섞인다
- 60일보다 오래된 영상은 뺀다. 최근 영상이 하나도 없으면 가장 관련도가 높은 1건만
  `stale` 표시와 함께 남긴다

공식 API 가 아니므로 구조가 바뀌면 조용히 빈 목록을 돌려주고 파이프라인은 계속 진행한다.
