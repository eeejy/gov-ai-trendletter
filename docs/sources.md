...
- **영상은 필수가 아니다.** 관련도가 확실하지 않으면 그 항목에는 아예 붙이지 않는다.
...
| --- | ---: |
| 제목 낱말 겹침 | **3개 이상** |
| 겹침 계산에서 뺄 상용어 | `AI`·`인공지능`·`모델`·`사용`·`활용`·`도입`·`개발`·`확산`·`정부`·`무료` 등 |
| 영상 나이 | 45일 이내 |
| 나이 예외 | 겹침 **4개 이상**이면 같은 사업·행사로 보고 오래돼도 인정 (`지난 회차` 표시) |

**왜 이렇게 올렸나.** 처음엔 겹침 2개면 채택했는데 상용어만 겹친 영상이 대부분이었다.

```
행정안전부, AI 친화적 보고서 서식 전면 도입
  → "행정안전부 | 국민 맞춤형 AI 비서와 정책 제안"   겹침 [AI, 행정안전부]
네이버클라우드·LG CNS, 보안 특화 소버린 AI 모델 개발
  → "LG CNS, 글로벌 AI 신소재…"                    겹침 [AI, CNS, LG]
해외 개발자 커뮤니티, 대형 AI 모델 무료 개방 확산
  → "완전 무료 AI로 24시간 돈 버는 에이전트 만들기"    겹침 [모델, 무료]
```

전부 주제가 다르다. 상용어를 빼고 3개 기준으로 올리자 6개 항목 중 1개에만
영상이 붙었고, 그 1건은 같은 행사의 지난 회차라 실제로 관련이 있었다.
